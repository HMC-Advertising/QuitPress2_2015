  ...
  <main role="main" class="main col-md-9">
    <div class="main-indent">
      ...
      <blockquote class="stories-quote"><?php $quote = get_field('quote');  if($quote) :  ?>
        <p><?php echo '"'.$quote.'"';  ?>
        </p><?php else:  ?><?php if( strstr(get_the_content(), "[audio") or strstr(get_the_content(), "<div style") ): ?><?php the_content(); ?><?php else : ?><?php echo '<p>"'.get_the_content().'"</p>';  ?><?php endif;	 ?><?php endif;  ?><?php $name = get_field('name');  ?><?php $location = get_field('location');  ?><?php if($location || $name) :  ?><span class="stories-quote-cite"><?php if($name):  ?><strong><?php echo $name; ?> </strong><?php endif;  ?><?php echo $location;  ?></span><?php endif;  ?><?php if($quote) :  ?><a href="<?php the_permalink(); ?>" class="button">Read More</a><?php endif;  ?>
