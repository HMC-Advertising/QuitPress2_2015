...
<main class="main">
  ...
  <div class="grid"><?php $quotes = get_field('quote_picker'); ?><?php if($quotes): ?>
    <div class="tgrid-2-3 mb0">
      <div class="main-indent">
        <div class="stories-quotes"><?php foreach($quotes as $post): ?><?php setup_postdata($post); ?><?php $quote = get_field('quote'); ?><?php $content = get_the_content(); ?><?php if((strpos($quote, '<iframe') !== FALSE) or strpos($content, '<iframe') !== FALSE): ?><?php if($quote): ?>
          ...
          <blockquote class="stories-quote f">
            <p>"<?php echo  $content; ?>"</p><span class="stories-quote-cite"><strong><?php the_field('name'); ?></strong><?php the_field('location'); ?></span>
          </blockquote><?php endif;  ?><?php endif;  ?><?php wp_reset_postdata(); endforeach; ?><a href="<?php $title_lower = strtolower(get_the_title()); echo home_url().'/real-stories/'.$title_lower; ?>" class="button-large">Read More Stories</a>
      ...
      </div><?php endforeach; ?><a href="<?php echo home_url();?>/resources/tips-and-tools/" class="button-large">Get More Tips &amp; Tools</a>
    </div><?php endif; ?>
  </div>
